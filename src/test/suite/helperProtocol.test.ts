...
interface ProtocolVector {
  id: string;
  kind: "hello" | "state" | "log";
  record: Record<string, unknown>;
  expected: Record<string, unknown> | null;
}

function loadProtocolVectors(): ProtocolVector[] {
  const file = path.resolve(__dirname, "../../../docs/helper-protocol-vectors.json");
  const document = JSON.parse(fs.readFileSync(file, "utf8")) as {
    schemaVersion: number;
    cases: ProtocolVector[];
  };
...
suite("helperProtocol", () => {
  test("matches the shared cross-client protocol vectors", () => {
    for (const vector of loadProtocolVectors()) {
      const line = JSON.stringify(vector.record);
      if (vector.kind === "hello") {
        const actual = parseHelloLine(line);
        if (vector.expected === null) {
          assert.equal(actual, undefined, vector.id);
        } else {
          assert.ok(actual, vector.id);
          assert.equal(actual?.version, vector.expected.version, vector.id);
          assert.deepEqual(actual?.capabilities, vector.expected.capabilities, vector.id);
        }
        continue;
      }

      if (vector.kind === "state") {
        const actual = parseSnapshotLine(line);
        if (vector.expected === null) {
          assert.equal(actual, undefined, vector.id);
        } else {
          assert.ok(actual, vector.id);
      ...
      const actual = parseLogLine(line);
      if (vector.expected === null) {
        assert.equal(actual, undefined, vector.id);
      } else {
        assert.ok(actual, vector.id);
  ...
  test("preserves only boolean raw state availability", () => {
    assert.equal(
      parseSnapshotLine(
    ...
    assert.equal(
      parseSnapshotLine(JSON.stringify({ type: "state", state: "unknown" }))?.rawStateAvailable,
      undefined
    );
    assert.equal(
      parseSnapshotLine(
