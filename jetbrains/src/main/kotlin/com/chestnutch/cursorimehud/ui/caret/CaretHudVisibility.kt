...
object CaretHudVisibility {
  ...
  ): CaretHudDisplayState = resolveSnapshot(snapshot, settings, editorAvailable, editorFocused)

  fun resolve(
    displayState: HudDisplayState,
    settings: CursorImeHudSettings.State,
    editorAvailable: Boolean,
    editorFocused: Boolean
  ): CaretHudDisplayState = resolveSnapshot(displayState.displaySnapshot, settings, editorAvailable, editorFocused)
  ...
  private fun resolveSnapshot(
    ...
    return CaretHudDisplayState(visible = true, label = label, snapshot = snapshot)
  }
}
