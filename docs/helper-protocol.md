...
| `conversionNative`  | boolean | no       | Windows only: whether the IME conversion mode (`ImmGetConversionStatus` / `IMC_GETCONVERSIONMODE`) has the `IME_CMODE_NATIVE` bit set. Sogou/WeChat/QQ flip this bit (not `isOpen`) for their internal Shift Chinese/English toggle. Omitted when the conversion mode cannot be read. Added as an optional field within protocol v1; per the compatibility rules below this does not bump `PROTOCOL_VERSION`. |
